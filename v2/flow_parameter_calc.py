...
def determine_misra_and_lund_flow_parameters(filename):
    global mu
...
def determine_flow_parameters(spectra_name='ml',filename="parameters_for_dhit_setup.txt"):
    if(spectra_name=='ml'):
        determine_misra_and_lund_flow_parameters(filename)
    else:
        print("ERROR: Invalid spectra passed to determine_flow_parameters(). Aborting...")
        exit()
    return

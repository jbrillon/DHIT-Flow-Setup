...
def convert_gl_to_equidistant_nodes(filename_without_extension,file_extension="dat"):
    ...
    nDOF_expected = raw_data.shape[0]
    if(nDOF!=nDOF_expected):
        print("Error: nDOF does not match expected nDOF from file %s, check var.py",input_file)
        print("Aborting...")
        exit()
    ...
    # file = open("dummy_delete_me.txt","w") # for testing
                ...
                for l in range(1-1, pp):
                    for m in range(1-1, pp):
                        for n in range(1-1, pp):
                            ...
                            #At Cartesian Points
                            Xg[indi,indj,indk] = (ri)*dh + (coeff1[p,l]+1.0)/2.0*dh;
                            Yg[indi,indj,indk] = (rj)*dh + (coeff1[p,m]+1.0)/2.0*dh;
                            Zg[indi,indj,indk] = (rk)*dh + (coeff1[p,n]+1.0)/2.0*dh;

                            #At Solution Points
                            Xs[indi,indj,indk] = (ri)*dh + (coeff2[p,l]+1.0)/2.0*dh;
                            Ys[indi,indj,indk] = (rj)*dh + (coeff2[p,m]+1.0)/2.0*dh;
                            Zs[indi,indj,indk] = (rk)*dh + (coeff2[p,n]+1.0)/2.0*dh;
    ...
    # WRITE TO FILE
    file = open(output_file,"w") # for testing
    for k in range(Nk1-1, Nk2):
        for j in range(Nj1-1, Nj2):
            for i in range(Ni1-1, Ni2):
...
# filename_without_extension="/Users/Julien/julien_phd/post_processing/data/taylor_green_vortex/2022-11-09_24dofs/viscous_TGV_ILES_cDG_IR_two_point_flux_dofs024_p5_procs16/flow_field_files/velocity_vorticity-0_reordered"
# filename_without_extension="/Users/Julien/julien_phd/post_processing/data/taylor_green_vortex/2022-11-09_48dofs/viscous_TGV_ILES_cPlus_IR_two_point_flux_dofs048_p5_procs64/flow_field_files/velocity_vorticity-0_reordered"
# filename_without_extension="/Users/Julien/julien_phd/post_processing/data/taylor_green_vortex/2022-11-09_48dofs/viscous_TGV_ILES_cDG_IR_two_point_flux_dofs048_p5_procs64/flow_field_files/velocity_vorticity-0_reordered"
...
data2 = np.loadtxt("/Users/Julien/DHIT-Flow-Setup/dofs048_p5_velocity/velocity_equidistant_nodes.fld",skiprows=0,dtype=np.float64)
file = open("diff_equidistant_nodes.dat","w")
